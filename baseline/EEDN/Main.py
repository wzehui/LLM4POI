...
def get_user_embeddings(model, user_dl, opt):
    ...
    valid_user_embeddings = torch.zeros((C.USER_NUMBER, opt.d_model),
            device='cuda:2')
...
def main(trial):
    ...
    if C.DATASET in lambda_delta:
        ...
        opt.lambda_, opt.delta = (trial.suggest_float('lambda', 0.1, 4),
                                  trial.suggest_float('delta', 0.1, 4))
