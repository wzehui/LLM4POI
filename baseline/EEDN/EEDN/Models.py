...
class Encoder(nn.Module):
    def __init__(
        ...
        if C.ENCODER == 'hGCN':
            ...
            self.ui_adj = torch.tensor(self.normalize_graph_mat(self.ui_adj).toarray(), device='cuda:2')
    ...
    def forward(self, event_type, enc_output, slf_attn_mask, non_pad_mask):
        ...
        if C.ENCODER == 'Transformer':
            ...
            # get individual adj
            adj = torch.zeros((event_type.size(0), event_type.size(1), event_type.size(1)), device='cuda:2')
