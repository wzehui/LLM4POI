...
def read_interaction_by_trajectory(user_trajectories):
    ...
    directory_path = '../../data/{dataset}/split/'.format(dataset=C.DATASET)
    # train_file = 'train.txt'.format(dataset=C.DATASET)
    # train_data = open(directory_path + train_file, 'r').readlines()
    count = 0

    interaction_matrix = torch.zeros((C.USER_NUMBER, C.POI_NUMBER), device='cuda:2')
    POI_matrix = torch.zeros((C.POI_NUMBER, C.POI_NUMBER), device='cuda:2')
...
def read_interaction(train_data=None, directory_path=None):
    ...
    if train_data is None:
        ...
        train_data.extend(open(directory_path + '{dataset}_val.txt'.format(
            dataset=C.DATASET), 'r').readlines())
    count = 0
    ...
    # print(POI_matrix)
    #np.save(directory_path + 'poi_matrix.npy', POI_matrix.cpu().numpy())
    np.save('./data/{dataset}/'.format(dataset=C.DATASET) + 'poi_matrix.npy', POI_matrix.cpu().numpy())
